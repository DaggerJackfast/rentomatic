...
def test_get(mock_use_case, client):
    ...
    assert json.loads(http_response.content.decode('UTF-8')) == [first_storageroom_dict]
...
def test_get_failed_response(mock_use_case, client):
    ...
    assert json.loads(http_response.content.decode('UTF-8')) == {'type': 'SYSTEM_ERROR',
...
def test_request_object_initialisation_and_use_with_filters(mock_use_case, client):
    ...
    with mock.patch(request_object_class) as mock_request_object:
        ...
        client.simulate_get(
            '/storagerooms',
            query_string='filter_param1=value1&filter_param2=value2',
        )
